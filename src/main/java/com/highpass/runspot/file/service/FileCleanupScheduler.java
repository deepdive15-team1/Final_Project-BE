...
public class FileCleanupScheduler {
    ...
    public void cleanup() {
        for (var file :
                files.findByStatusAndCreatedAtBefore(
                        UploadedFileStatus.PENDING, LocalDateTime.now().minusHours(24))) {
            s3.deleteObject(
                    DeleteObjectRequest.builder().bucket(bucket).key(file.getImageKey()).build());
            files.delete(file);
        }
    }
}
