...
public record RunningRecordResponse(
        ...
        String mapThumbnailKey,
        String routePolyline) {
    public static RunningRecordResponse from(RunningRecord r) {
        int sec = r.getAvgPaceSec();
        return new RunningRecordResponse(
                r.getId(),
                r.getTitle(),
                r.getRecordedAt(),
                r.getDistanceKm(),
                String.format("%d:%02d/km", sec / 60, sec % 60),
                r.getLocationName(),
                r.getLocationDetail(),
                r.getMapThumbnailKey(),
                r.getRoutePolyline());
    }
}
